...
} from "../../../../../resources/js/React/components/ProductImageUploader/ProductImageUploader";

export default function ProductCreateForm({ onCreate }: ProductCreateFormProps) {
  const [name, setName] = useState("");
  const [price, setPrice] = useState("");
  const [errorMsg, setErrorMsg] = useState<string | null>(null);
  const [createdProduct, setCreatedProduct] = useState<{ id: number | string } | null>(null);
  ...
  const onCreateHandler = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    // armazena o produto criado (para usar o id no uploader)
    setCreatedProduct(data);

    // habilita o componente de upload
    ...
    <div className="col-12 col-lg-4">
      <div className="card border-0 shadow-sm rounded-4 h-100">
        <div className="card-body">
          ...
          {createdProduct && (
            <div className="mt-4">
              <h6>Enviar imagem do produto:</h6>
